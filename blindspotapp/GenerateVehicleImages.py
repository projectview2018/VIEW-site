...
#put in its own method in case the "generate overhead image upon step 10 being done" is ever re-implemented
def get_data_top_combined( abd, data, id_num):
    ...
    if not "processed" in data.keys() or data['processed'] == 0:
        ...
        main_list = [0] * 360
        num_dupl = [0] * 360
        print("get_data_top_combined, data: ", data)
...
def add_VRU_top(fig, data, scale, s, index = 0, fast = True):
    ...
    img = Image.new("RGBA", (200 * s, 200 *s), (255, 255, 255, 0)) # the second parameter is a (width, height) tuple
    ...
    #if the VRU is a bicyclist, the length_increment will be 4
    #otherwise, length_increment will be 1
    length_increment = 1
    ...
    for i in range(width_boxes):
        #possible_y = True
        for j in range(0, height_boxes, length_increment):
            ...
            box = [ bounding_box[0] + i * p_width,  bounding_box[1] + j * p_width, int( bounding_box[0] + (i + 1) * p_width), int( bounding_box[1] + (j + length_increment) * p_width )]
            #print(box)
...
def create_all_images(id_num, scale, index):
    ...
    if data is not None:
        ...
        if index >= 0:
            add_VRU_top(fig, data, s, scale , index)
        #top_str = fig
...
# used to create overhead upon getinfo page load
def create_overhead_only(id_num, scale, index):
    #create overhead
    print("starting create_overhead_only")
    scale = int(scale)
    data = get_data_top(id_num)
    print("data", data)
    if data is not None:
